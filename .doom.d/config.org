...
** corfu-tng 与 eglot 的兼容性
corfu 的作者认为 =:exit-function= 的 =exact= 状态表示什么都不做，而 =finished= 表示进行操作。
但是 eglot 和 lsp-mode 都认为 =exact= 与 =finish= 一样，都会进行操作（比如 expand-snippet ）。

这就导致了 corfu 开启 corfu-preview-current 后，会自动触发 lsp server 的 snippet。
这并不是代码不能实现，或者有什么离奇的实现，而是类似“意识形态”的不同导致的系统间对接的bug
不是没法改，而是不想改。参见[[https://github.com/minad/corfu/issues/145][Corfu Issue#145]]

修改 corfu 的代码使它连 =exact= 状态都不传就能解决问题。
#+begin_src emacs-lisp
  (after! corfu
