...
取消一些快捷键。
这是一些我用不上的插件。没在init.el中勾选却还是显示在了快捷键中。
故在此将其取消。
#+begin_src emacs-lisp
  (map! :leader
        "f c" nil
...
解决在dirvish中toggle shell返回时聚焦在预览窗口。
#+begin_src emacs-lisp
...
** indent-guides
缩进提示是一个很好用的功能。
奈何emacs无可救药的单线程，描绘缩进会造成卡顿...
所以抄了一些优化代码试一试。
来源: https://github.com/seagle0128/.emacs.d/blob/aba20787f13f54ee144e6e3d2abb1d6560f7f0e2/lisp/init-highlight.el#L136
修改了mode名，因为doom里在prog-mode, text-mode, conf-mode里都开了indent-guide
#+begin_src emacs-lisp
...
不在markdown-mode中打开indent-guides
#+begin_src emacs-lisp
